...
from models.orthogonal_v2 import Orthogonal
...
class BayesDiagLaplacianBuilder(LaplacianBuilder):
    """Learns a a Sheaf Laplacian with diagonal restriction maps"""

    def __init__(self, size, edge_index, d, normalised=False, deg_normalised=False, add_hp=False, add_lp=False,
                 augmented=True):
        super(BayesDiagLaplacianBuilder, self).__init__(
    ...
    def forward(self, mean_params, std_params):

        err = torch.normal(0, 1, size=mean_params.size(), dtype=torch.float, device=std_params.device)
        maps = mean_params + (std_params * err)
...
class BayesNormConnectionLaplacianBuilder(LaplacianBuilder):
    """Learns a Sheaf Laplacian with special orthogonal restriction maps"""

    def __init__(self, size, edge_index, d, add_hp=False, add_lp=False, orth_map=None, augmented=True):
        super(BayesNormConnectionLaplacianBuilder, self).__init__(
        ...
        # self.cayley = Orthogonal(d=self.d, orthogonal_map='cayley')
        self.random_so = special_ortho_group(self.d)
    ...
    def forward(self, mean_params, var_params, edge_weights=None):
        if edge_weights is not None:
            assert edge_weights.size(1) == 1
        assert len(mean_params.size()) == 2
        assert mean_params.size(1) == self.d * (self.d - 1) // 2
        assert var_params.size(1) == 1
        ...
        rso = torch.tensor(self.random_so.rvs(size=mean_params.size(0)), device=mean_params.device, dtype=torch.float)
        var_maps = self.cayley_transform(var_params.view(var_params.size(0), 1, 1) * self.cayley_transform(rso))
        mean_maps = self.orth_transform(mean_params)
        maps = torch.bmm(var_maps, mean_maps)
...
class BayesGeneralLaplacianBuilder(LaplacianBuilder):
    ...
    def __init__(self, size, edge_index, d, normalised=False, deg_normalised=False,
        ...
        super(BayesGeneralLaplacianBuilder, self).__init__(
    ...
    def forward(self, mean_params, std_params):
        ...
        err = torch.normal(0, 1, size=mean_params.size(), dtype=torch.float, device=std_params.device)
        maps = mean_params + (std_params * err)
        maps = maps.reshape(maps.size(0), self.d, self.d)
